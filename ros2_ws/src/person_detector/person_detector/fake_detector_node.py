#!/usr/bin/env python3
"""
Fake detector using p3d odometry topics for Go2 and patient.

Simulation only — subscribes to both /odom (Go2) and /patient/odom (patient).
"""
...
class FakeDetectorNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f'FakeDetector: HFOV={math.degrees(self.hfov):.0f}deg, '
            f'max_range={self.max_range}m'
        )

    def go2_cb(self, msg: Odometry):
    ...
    def publish_target(self):
        ...
        self.get_logger().info(
            f'Patient at x={x_base:.2f} y={y_base:.2f} (dist={dist:.2f}m)',
