#!/usr/bin/env python3
"""
MPPI-based follow controller.
Subscribes to person_follow/target (base_link) and sends FollowPath
action goals to Nav2 controller_server, which uses MPPI to compute cmd_vel.
...
class FollowControllerNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f'FollowController (MPPI) ready. follow_dist={self.follow_dist}m'
        )
    ...
    def mode_cb(self, msg):
        prev_mode = self.mode
        self.mode = msg.mode
        if prev_mode == RobotMode.FOLLOW and self.mode != RobotMode.FOLLOW:
            self._cancel_goal()
    ...
    def control_loop(self):
        ...
        # Goal: follow_dist behind person along bearing (robot frame)
        gx_robot = (dist - self.follow_dist) * math.cos(bearing)
        gy_robot = (dist - self.follow_dist) * math.sin(bearing)
        goal_pose = self._robot_to_odom(gx_robot, gy_robot, bearing)

        # Throttle by position delta
    ...
    def _send_follow_path(self, goal_pose):
        if not self._action_client.wait_for_server(timeout_sec=0.1):
            self.get_logger().warn('controller_server not available', throttle_duration_sec=2.0)
            return

        # Build simple 2-point path: current pose -> goal
        odom = self.current_odom
        start_pose = PoseStamped()
        start_pose.header = goal_pose.header
